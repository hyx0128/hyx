...
# >>> NEW <<< Emotion Recognition Configuration
#加载cnn模型
...
emotion_classifier = load_model(EMOTION_MODEL_PATH, compile=False,custom_objects={"mini_XCEPTION": mini_XCEPTION})
...
# ASR Model
#AutoModel是一个通用的模型加载类，用于根据配置自动实例化预训练的语音识别模型。
asr_model = AutoModel(
    model=MODEL_DIR,
        ...
        while self.running:
            try:
                ...
                if len(faces) > 0:
                    ...
                    self.app.last_emotion = emotion_label
                else:
                    self.app.last_emotion = "neutral"
...
class MultimediaAssistantApp(ctk.CTk):
    def __init__(self):
        ...
        #>>>>NEW>>>>>
        # 先定义系统消息
        self.system_message = {"role": "system", "content": """"你是一个温柔体贴的学习/工作陪伴助手，不是监督者。"
        "你需要像朋友一样陪伴用户，安慰、鼓励、关心他们。"
        "你的回应要口语化、自然、简短（20~40字），不要像命令。"
        "当用户分心时，不要批评，而是轻声提醒：‘要不要一起专注一会？’。"
        "当用户疲惫时，建议他们休息：‘辛苦啦，休息一下吧，我等你。’。"
        "当用户情绪低落时，安慰他们：‘我在呢，不要担心。’。"
        "当用户情绪积极时，和他们分享喜悦：‘哈哈真棒，你看起来很开心！’。"
        """}
        ...
        #主动关怀冷却
        self.last_reply_time = time.time()    # >>> NEW: 上次回应时间
        self.last_auto_time = time.time()   #主动关怀
        ...
        if placeholder_id in self.placeholder_map:
            ...
            if placeholder_id.startswith("img_"):
                ...
                try:
                    ...
                    # >>> 0. 主动关怀（每隔40分钟触发）
                    if hasattr(self, "last_auto_time") and time.time() - self.last_auto_time > 2400:
                        care_reply = "你已经专注很久啦，要不要休息一下，喝点水？"
                        self.add_ai_message(care_reply)
                        self.audio_player.play_text(care_reply)
                        self.last_auto_time = time.time()
                        del self.placeholder_map[placeholder_id]
                        return
                    
                    # >>> 1. 情绪安慰（立即触发，不受冷却限制）
                    if self.last_emotion in ["sad", "angry", "scared", "disgust"]:
                        emotion_care = {
                            "sad": "我感觉到你有些难过，不要担心，我会一直陪着你。",
                            "angry": "别太生气啦，深呼吸一下，我们慢慢来。",
                            "scared": "放松点哈，我在这儿陪你，没有什么好怕的。",
                            "disgust": "我理解你此刻的不适感，别太在意，我会陪着你。"
                        }
                        reply = emotion_care[self.last_emotion]
                        self.add_ai_message(reply)
                        self.audio_player.play_text(reply)
                    
                        del self.placeholder_map[placeholder_id]
                       
                    # >>> 2.行为提醒
                    #走神经常提醒
                    if ( any(keyword in behavior_desc for keyword in ["分心", "手机", "看手机", "玩手机", "低头", "滑屏", "刷屏"])):
                        reply = "看起来你有些走神啦，要不要重新集中一下？"
                        self.add_ai_message(reply)
                        self.audio_player.play_text(reply)
                    ...
                    if (any(keyword in behavior_desc for keyword in ["疲惫", "打瞌睡", "困", "闭眼", "睡", "趴桌子"])):
                        reply = "你看起来有点累了，休息一下会更有效率哦。"
                        self.add_ai_message(reply)
                        self.audio_player.play_text(reply)
                        self.last_reply_time = time.time()
                        del self.placeholder_map[placeholder_id]
                    ...
                    if (any(keyword in behavior_desc for keyword in ["吃", "食", "吃东西", "吃饭", "零食", "喝", "饮食", "喝水"])):
                        reply = "现在是学习/工作时间呢，吃点东西后别忘了继续加油呀～"
                        self.add_ai_message(reply)
                        self.audio_player.play_text(reply)
                        self.last_reply_time = time.time()
                        del self.placeholder_map[placeholder_id]
                    ...
                    # >>> 3. 冷却逻辑（15 分钟内不重复回应）
                    if hasattr(self, "last_reply_time") and time.time() - self.last_reply_time < 900:
                        del self.placeholder_map[placeholder_id]
                        return
                    ...
                    # >>> NEW <<< 正常走大模型
                    emotion_care = {
                        "happy": "你看起来心情很好，真为你高兴！",
                        "surprised": "好像有让你意外的事情呢，真有趣！",
                        "neutral": "我在关注你，有什么想说的都可以告诉我。"
                    }
                    care_text = emotion_care.get(self.last_emotion, "我在陪你，一起加油吧。")
                    ...
                    messages = [
                        self.system_message,
                        {
                            "role": "user",
                            "content": (
                                f"当前观察到的行为: {new_content}。\n"
                                f"检测到的情绪: {self.last_emotion}。\n"
                                f"请结合关怀语: {care_text}\n"
                                f"生成一句温柔、主动关怀的陪伴回应。\n"
                                f"要求：\n"
                                f"1. 行为上：学习/工作 → 鼓励；分心 → 温柔提醒；休息/疲惫 → 安慰健康。\n"
                                f"2. 情绪上：消极 → 安慰陪伴；积极 → 认可鼓励；neutral → 正常陪伴。\n"
                                f"3. 用口语化的语气，简短（20~40字），像朋友聊天，不要机械。"
                            )
                        }
                    ]
                    ...
                    self.last_reply_time = time.time()#更新时间
